...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'TlsConfig.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'TlsConfig.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'TlsConfig.Client.ProtectedToken'
