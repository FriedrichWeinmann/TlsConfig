...
$parentModule = 'TlsConfig'
if (-not $ModuleName) { $ModuleName = 'TlsConfig.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\TlsConfig'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\TlsConfig.psd1"
$prereqHash = @{
